<template>
  <div class="max-w-4xl">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="font-serif text-2xl text-[#1a1917] italic font-normal">Clientes</h2>
        <p class="text-xs text-[#aaa] mt-0.5">
          {{ pagination.total ? `${pagination.total} cliente${pagination.total !== 1 ? 's' : ''} cadastrado${pagination.total !== 1 ? 's' : ''}` : 'Gerencie sua base de clientes' }}
        </p>
      </div>
      <button
    ...
    <transition name="slide">
      <div v-if="showForm" class="bg-white border border-[#e8e5df] rounded-xl p-5 mb-6">
        <p class="text-xs text-[#aaa] uppercase tracking-widest mb-4 font-medium">
        ...
        <form @submit.prevent="save" class="grid grid-cols-2 gap-3">
          <input
            v-model="form.name"
            placeholder="Nome"
            class="col-span-2 h-9 px-3 text-sm bg-[#faf9f7] border border-[#e8e5df] rounded-lg text-[#1a1917] placeholder-[#ccc] focus:outline-none focus:border-[#1a1917] transition-colors"
          />
          <input
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="bg-[#faf9f7] border-b border-[#e8e5df]">
            ...
            <th class="text-left text-[10px] text-[#aaa] uppercase tracking-widest font-medium px-4 py-3">Documento</th>
            <th class="text-left text-[10px] text-[#aaa] uppercase tracking-widest font-medium px-4 py-3">Email</th>
            <th class="text-left text-[10px] text-[#aaa] uppercase tracking-widest font-medium px-4 py-3">Status</th>
        ...
        <tbody>
          ...
          >
            <td class="px-4 py-3 font-medium text-[#1a1917]">{{ c.name }}</td>
            <td class="px-4 py-3 text-[#888]">{{ c.document }}</td>
            <td class="px-4 py-3 text-[#888]">{{ c.email }}</td>
            <td class="px-4 py-3">
...
export default {
  data() {
    return {
      ...
      form: { id: null, name: "", document: "", email: "", status: "active" },
    };
  },
  mounted() {
    this.load();
  ...
  methods: {
    async load(page = 1) {
      const { data } = await api.get("/v1/clients", { params: { page } });
    ...
    reset() {
      this.form = { id: null, name: "", document: "", email: "", status: "active" };
    },
    statusClass(status) {
      const base = "inline-flex items-center text-[11px] font-medium px-2.5 py-0.5 rounded-full";
      return status === "active"
        ? `${base} bg-[#f0fdf4] text-[#166534]`
        : `${base} bg-[#f5f4f0] text-[#888]`;
    },
  },
};
</script>
